    ...
    while (emuStatus != emuStatus_Exit)
    {
        ...
        if (emuStatus == emuStatus_Running || emuStatus == emuStatus_FrameStep)
        {
            ...
            if (nframes >= 30)
            {
                ...
                snprintf(melontitle, sizeof(melontitle), "[%d/%.0f] Water Melon Mix " MELONDS_VERSION, fps, actualfps);
        ...
        else
        {
            ...
            snprintf(melontitle, sizeof(melontitle), "Water Melon Mix " MELONDS_VERSION);
            changeWindowTitle(melontitle);
